...
from hist import RESOURCE_PATH
from hist import START_MONTH
from hist import async_download_file  # Downloading
from hist import async_get_daily_list  # Data listing
from hist import async_get_monthly_list
from hist import async_get_usdt_symbols  # Symbol management
from hist import batch_process_data  # Data processing
from hist import clean_old_daily_zip
from hist import get_download_prefix
from hist import get_latest_parquet
from hist import get_local_path  # File management
from hist import need_analyse_set
from hist import proxy  # Config
from hist import spot_symbols_filter
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.get(
                url="https://fapi.binance.com/fapi/v1/ping", proxy=proxy, timeout=5
            ) as response:
...
def download_trade_type_data(trade_type, start_time, interval="5m"):
    ...
    logger.info(
        f"{trade_type} 所有数据包个数为 {len(all_list)}，获取目录耗费 {(get_time - start_time).seconds} s"
    )
    ...
    logger.info(
        f"{trade_type} download end cost {(end_time - get_time).seconds} s = {(end_time - get_time).seconds / 60} min"
    )
...
def run(interval="5m"):
    ...
    logger.info(
        f"现货数据下载耗时: {(usdt_spot_end_time - usdt_perp_end_time).seconds / 60:.2f} 分钟"
    )
    ...
    for market in ["usdt_perp", "usdt_spot"]:
        try:
            ...
            with timer(f"写入 {market}_{KLINE_INTERVAL} 数据"):
                max_time = db_manager.fetch_one(
                    f"""select max(open_time) from read_parquet('{file_path}')"""
                )[0]
