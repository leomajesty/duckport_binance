...
from utils.config import create_download_semaphore
from utils.config import daily_updated_set
from utils.config import file_proxy
from utils.config import need_analyse_set
from utils.config import retry_times
from utils.config import thunder
...
def download_file(params, pbar, error_info_list, sem):
    ...
    for param in params:
        ...
        if os.path.exists(local_sum_path):
            ...
            modify_utc_timestamp = datetime.datetime.utcfromtimestamp(
                os.path.getmtime(local_sum_path)
            ).timestamp()
...
async def download(
    local_path, download_checksum_url, local_sum_path, local_zip_path, pbar, error_info_list, sem
):
        ...
        async with sem:
            ...
            while True:
                if retry > retry_times and "daily_klines" in local_path:
                    logger.warning(
                        "下载daily zip失败次数超过retry_times，当前网络状况不稳定或数据包异常",
                        local_zip_path,
                    )
                    break
                try:
                    sum_file = await session.get(
                        download_checksum_url, proxy=file_proxy, timeout=20
                    )
                    ...
                    zip_file = await session.get(
                        download_checksum_url[0:-9], proxy=file_proxy, timeout=20
                    )
                    ...
                    error_info_list.add(
                        f"下载{local_zip_path}失败，错误原因{ae}，已重试下载，请确认"
                    )
                except Exception as e:
                    error_info_list.add(
                        f"下载{local_zip_path}失败，错误类型{type(e)}，已重试下载，请确认"
                    )
                retry += 1


async def download_miss_day_data(
    symbol, interval, day, local_path, sum_name, prefix, download_err_info, sem
):
    ...
    if retry_sum_404 > 3 or retry_zip_404 > 3:
        ...
        logger.warning(
            f"Missing data for {symbol} on {day}, API fallback not implemented in simplified version"
        )
