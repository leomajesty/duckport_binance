...
from utils.config import metrics_prefix
from utils.log_kit import logger

from .file_manager import get_local_path
from .symbol_manager import request_session
from .symbol_manager import request_session_4_list
...
def async_get_daily_list(download_folder, symbols, trading_type, data_type, interval):
    ...
    return asyncio.run(
        download_daily_list(download_folder, symbols, trading_type, data_type, interval)
    )
...
async def download_daily_list(download_folder, symbols, trading_type, data_type, interval):
    ...
    for symbol in symbols:
        ...
        checksum_file_name = (
            f"{symbol.upper()}-{interval}-{daily_end - relativedelta(days=1)}.zip.CHECKSUM"
        )
        first_checksum_file_uri = f"{daily_prefix}{checksum_file_name}"
        param = {"delimiter": "/", "prefix": daily_prefix, "marker": first_checksum_file_uri}
    ...
    for task in tasks:
        ...
        local_path = get_local_path(
            download_folder, trading_type, data_type, "daily", symbol, interval
        )
...
async def build_download_monthly_list(download_folder, symbols, trading_type, data_type, interval):
    ...
    for symbol in symbols:
        ...
        param = {"delimiter": "/", "prefix": monthly_prefix}
        param_list.append(param)
    ...
    for task in tasks:
        ...
        symbol = root.Prefix.text.split("/")[-3]
        local_path = get_local_path(
            download_folder, trading_type, data_type, "monthly", symbol, interval
        )
        for item in root.Contents:
            key = item.Key.text
            if (
                key.endswith("CHECKSUM")
                and (key[-20:-13] <= end_month)
                and (key[-20:-13] >= start_month)
            ):
                # Apply START_DATE filtering
                if is_after_start_date(key):
                    struct_time = time.strptime(item.LastModified.text, "%Y-%m-%dT%H:%M:%S.%fZ")
...
async def build_download_metrics_list(download_folder, symbols, trading_type, data_type, interval):
    ...
    for symbol in symbols:
        ...
        param = {"delimiter": "/", "prefix": symbol_prefix}
        param_list.append(param)
