...
from utils.date_partition import get_available_years_months
    ...
    for symbol in symbols:
        ...
        if os.path.exists(local_daily_symbol_path):
            zip_file_path = os.path.join(
                local_daily_symbol_path, f"{symbol.upper()}-{interval}-{daily_end}.zip"
            )
...
def to_pqt(yms: list[str], interval: str = "5m", market: str = "usdt_perp"):
    ...
    if (
        os.path.exists(os.path.join(PARQUET_DIR, f"{market}_{interval}", filename))
        and filename != latest_pqt
    ):
    ...
    results = Parallel(n_jobs=CONCURRENCY)(
        delayed(process_symbol_month)(syb, ydashm) for syb in tqdm(symbols) for ydashm in yms
    )
