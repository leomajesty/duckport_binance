...
from core.api.exceptions import BinanceWebsocketUnableToConnect
from utils import get_loop, get_logger
...
class ReconnectingWebsocket:
    ...
    def _create_connection_kwargs(self):
        """创建websocket连接的参数"""
        kwargs = {'close_timeout': 0.1}
    ...
    async def connect(self):
        ...
        # 使用正确的连接参数
        connection_kwargs = self._create_connection_kwargs()
        self._conn = ws.connect(ws_url, **connection_kwargs)  # type: ignore

        try:
            self.ws = await self._conn.__aenter__()
        except Exception as e:
            self._log.error(f"Failed to connect to {ws_url}: {e}")
