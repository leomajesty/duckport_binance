...
from core.component.candle_fetcher import OptimizedKlineFetcher
from core.component.candle_listener import MarketListener
from core.flight_func.flight_api import FlightActions
from core.flight_func.flight_api import FlightGets
from utils import async_sleep_until_run_time
...
class DataJobs:
    def __init__(
        self, db_manager: KlineDBManager, flight_actions: FlightActions, flight_gets: FlightGets
    ):
            ...
            async with create_aiohttp_session(10) as session:
                ...
                for market in ["usdt_perp", "usdt_spot"]:
                    ...
                    if latest_times[market]:
                        await self._check_symbol_consistency(
                            market, current_symbols[market], latest_symbols[market]
                        )
        ...
        for market in ["usdt_perp", "usdt_spot"]:
            try:
                ...
                if duck_time and duck_time != GENESIS_TIME:
                    ...
                    latest_symbols[market] = self.get_trading_symbols_by_time(
                        latest_times[market], market, is_duck=True
                    )
                elif pqt_time and pqt_time != GENESIS_TIME:
                    latest_times[market] = pqt_time
                    latest_symbols[market] = self.get_trading_symbols_by_time(
                        latest_times[market], market, is_duck=False
                    )
                else:
    ...
    def get_trading_symbols_by_time(self, snaptime, market, is_duck=True):
        ...
        if is_duck:
            ...
            df = self._db_manager.fetch_df(
                f"select symbol from {market}{SUFFIX} where open_time = ('{snaptime_str}'::timestamp - '{KLINE_INTERVAL}'::interval)"
            )
    ...
    async def _update_historical_klines(self, fetcher, symbols, start_time, market, current_time):
        ...
        res = await optimized_fetcher.get_all_klines(
            symbols, start_time=start_time, interval=KLINE_INTERVAL, limit=499
        )
    ...
    async def save_exginfo(self, fetcher, market):
        ...
        try:
            ...
            logger.info(
                f"symbols_trading数据已保存到数据库: {market}, {len(symbols_trading_df)} 条记录"
            )
        except Exception as e:
    ...
    def duckdb_retention_policy(self):
        ...
        async def periodic():
            while True:
                ...
                divider(
                    f"[Scheduler] next retention job runtime: {next_time:%Y-%m-%d %H:%M:%S}",
                    display_time=False,
                )
            ...
            try:
                ...
                if pqt_info and not pqt_info["exists"]:
                    ...
                    with timer(f"{market} retention task: {filename}"):
                        # 写入pqt
                        pqt_file_path = os.path.join(
                            PARQUET_DIR, f"{market}_{KLINE_INTERVAL}", filename
                        )
    ...
    async def _duckdb_periodic_fetch_async(self):
        ...
        divider(
            f"[Scheduler] next fetch runtime: {next_time:%Y-%m-%d %H:%M:%S}", display_time=False
        )
        await async_sleep_until_run_time(next_time)
        try:
            await self._fetch_and_insert_binance_data_async(
                market="usdt_perp", current_time=next_time, interval=KLINE_INTERVAL
            )
            await self._fetch_and_insert_binance_data_async(
                market="usdt_spot", current_time=next_time, interval=KLINE_INTERVAL
            )
        except Exception as e:
            logger.error(f"Scheduler Error: {e}")
    ...
    async def _fetch_and_insert_binance_data_async(self, market, current_time, interval="5m"):
        ...
        async with create_aiohttp_session(10) as session:
            ...
            if gap_time <= timedelta(minutes=KLINE_INTERVAL_MINUTES * 99):
                results = await optimized_fetcher.get_all_klines(
                    symbols_trading, interval=interval, limit=99
                )
            elif (
                gap_time <= timedelta(minutes=KLINE_INTERVAL_MINUTES * 499)
                or self._flight_actions.duck_time[market] == GENESIS_TIME
            ):
                results = await optimized_fetcher.get_all_klines(
                    symbols_trading, interval=interval, limit=499
                )
            else:
        ...
        while True:
            try:
                ...
                async def data_callback(run_time: datetime, batch_data: dict, _market: str):
                    """处理批次数据的回调函数"""
                    if run_time - self._flight_actions.duck_time[_market] > timedelta(
                        minutes=KLINE_INTERVAL_MINUTES
                    ):
                        logger.warning(
                            f"do remedy. [{market}] runtime: {run_time}, ducktime: {self._flight_actions.duck_time[_market]}"
                        )
                        await self._fetch_and_insert_binance_data_async(
                            market=_market, current_time=run_time, interval=KLINE_INTERVAL
                        )
                        return
                    market_df = []
                    ...
                    if market_df is not None:
                        ...
                        logger.debug(
                            f"已写入 {_market} 市场数据{len(market_df)}条: at {now_time()}"
                        )
        ...
        try:
            ...
            converted_data = {
                ...
                "taker_buy_base_asset_volume": [
                    float(df_candle["taker_buy_base_asset_volume"].iloc[0])
                ],
                "taker_buy_quote_asset_volume": [
                    float(df_candle["taker_buy_quote_asset_volume"].iloc[0])
                ],
                "avg_price": [
                    float(df_candle["quote_volume"].iloc[0]) / float(df_candle["volume"].iloc[0])
                    if float(df_candle["volume"].iloc[0]) > 0
                    else 0.0
                ],
            }
            ...
            logger.error(
                f"转换市场数据失败: {e}, market: {market}, symbol: {kline_data.get('symbol', 'unknown')}"
            )
            return None
