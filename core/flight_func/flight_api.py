...
class FlightActions:
    ...
    def action_ready(self, market, **kwargs):
        ...
        return [flight.Result(pa.scalar(str(duck_time)).as_buffer())]
...
class DataJobs:
    ...
    def init_history_data(self):
        ...
        async def history_data():
            ...
            async with create_aiohttp_session(10) as session:
                ...
                for market in ['usdt_perp', 'usdt_spot']:
                    if latest_times[market]:
                        await self._update_historical_klines(fetchers[market], current_symbols[market], latest_times[market], market, current_time)
    ...
    def get_trading_symbols_by_time(self, snaptime, market, is_duck=True):
        if is_duck:
            ...
            df = self._db_manager.fetch_df(f"SELECT symbol from read_parquet('{self._flight_gets._pqt_path}/{market}{SUFFIX}/*.parquet') where open_time = '{snaptime_str}'")
            return df['symbol'].to_list()
    ...
    def write_kline(self, df, market, current_time):
        with timer("write to duckdb"):
            # 写入数据库忽略重复数据
            self._db_manager.execute_write(f"insert into {market}_{KLINE_INTERVAL} select * from df on conflict do nothing;",
                                           df=df)
    ...
    def update_recent_data(self, if_ws: bool = False):
        """更新近期数据"""
        if if_ws:
            logger.info('waiting for remedy')
            asyncio.run(self._duckdb_periodic_fetch_async())
            logger.info('websocket subscript mode')
            # TODO
        else:
            logger.info('restful API mode')
            fetch_job = threading.Thread(target=self.duckdb_periodic_fetch_policy, daemon=True)
            fetch_job.start()
            retention_job = threading.Thread(target=self.duckdb_retention_policy, daemon=True)
            retention_job.start()
